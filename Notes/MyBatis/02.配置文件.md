...
\<configuration> 为根标签，MyBatis 其余元素必须在根标签内有序配置。
...
| 属性名                   | 描述                                 | 属性值        | 默认值 |
| ------------------------ | ------------------------------------ | ------------- | ------ |
| cacheEnabled             | 开启或关闭所有映射文件中已配置的缓存 | true \| false | true   |
...
在 \<databaseIdProvider> 标签内定义不同的数据库厂商。value 值被映射文件中的 databaseId 属性引用，MyBatis 根据不同的数据库厂商执行不同的语句。
