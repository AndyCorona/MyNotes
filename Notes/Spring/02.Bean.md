...
Spring 容器负责创建应用程序中的 bean 并通过依赖注入协调这些对象之间的关系。Spring 提供了三种主要的装配方式，告诉 Spring 要创建哪些 bean 并如何装配。
- 在 XML 中进行显式配置。
- 在 Java 中进行显式配置。
- 隐式的 bean 发现机制和自动装配。
...
ConcreteCD 类实现 CD 接口，@Component 注解表明该类是一个组件类，并告知 Spring 要为这个类创建 bean。Spring 应用上下文中的所有 bean 都有一个 ID。bean 默认 ID 为其类名的首字母小写形式，如 ConcreteCD 类创建的 bean 的 ID 默认为 concreteCD。在 @Component 注解中可以配置自定义的 bean ID。来自 Java 依赖注入规范的 @Named 注解也能实现相同功能。
...
开启组件扫描后，Spring 容器会寻找带有 @Component 注解的组件类，并为其创建 bean。但组件扫描默认是关闭的，需要用 @ComponentScan 注解开启组件扫描。CDPlayerConfig 是一个开启组件扫描的配置类。@ComponentScan 注解默认以配置类所在的包为基础包（如配置类 CDPlayerConfig 所在的 Autowired 包为基础包），扫描基础包及其所有子包，查找带有 @Component 注解的类，并创建 bean。
...
自动装配是让 Spring 自动满足 bean 依赖的一种方法。CDPlayer 类 依赖 CD 接口，其构造器上的 @Autowired 注解告诉 Spring 当创建 CDPlayer 的 bean 时，必须注入一个实现了 CD 接口的 bean，通过构造方法完成依赖注入。来自  Java 依赖注入规范的 @Inject 注解也能实现自动装配功能。
...
- 若只有一个 bean 匹配时，这么 bean 就会被装配进来，实现依赖注入；
- 若没有匹配的 bean，在应用上下文创建的时候，Spring 会抛出一个异常。为了避免异常，可以将 @Autowired 的 required 属性设置为 false，若没有匹配的 bean，Spring 会让这个 bean 处于未装配的状态，小心，此时很有可能发生空指针异常；
- 若有多个匹配的 bean，Spring 将抛出一个异常，表明没有明确指定要选择哪个 bean 进行装配。
...
@Configuration 注解指明 CDPlayerConfig 为配置类，它不包含任何逻辑业务，只有创建 bean 和注入依赖的功能。
@Bean 注解指明该方法将创建一个对象，注册为 Spring 上下文中的 bean。和 @Component 注解类似，可以自定义 bean ID。getCD() 方法创建 ConcreteCD bean，cdPlayer() 方法创建 CDPlayer bean，并注入依赖。
```java
@Configuration
public class CDPlayerConfig {
...
在 xml 文件中装配 bean 时，以 <beans> 为根元素，用 <bean> 元素声明一个 bean。bean 默认 ID 为 “全类名#n” 形式，id 属性可自定义 bean ID。用 <constructor-arg> 元素实现构造器注入，ref 属性指明要注入的 bean 的 ID。
...
```
c:cd-ref="idBean" 中，c 代表 c 命名空间前缀，cd 代表构造器参数名，ref 代表注入 bean 引用，后接要注入 bean 的 ID。此时我们引用了具体的构造器参数名，若参数名发生改变，配置将失效。
...
```
或用 c 空间向构造器注入参数值，c 空间不能直接引用集合，需要 util 空间创建集合 bean，然后引用该集合 bean。
...
```
用 <property> 元素实现属性 setter 方法注入。name 属性代表注入的属性名，ref 属性指明要注入 bean 的 ID。
...
```
p:cd-ref="idBean" 中，p 代表 p 命名空间前缀，cd 代表注入的属性名，ref 代表注入 bean 引用，后接要注入 bean 的 ID。
...
或用 p 空间注入参数值，p 空间不能直接引用集合，需要 util 空间创建集合 bean，然后引用该集合 bean。
